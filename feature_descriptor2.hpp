//@	{"targets":[{"name":"feature_descriptor2.hpp","type":"include"}]}


#ifndef LV2PLUG_FEATUREDESCRIPTOR2_HPP
#define LV2PLUG_FEATUREDESCRIPTOR2_HPP
...
namespace LV2Plug
	...
	namespace
		...
		static constexpr uint64_t make_key(const char* str)
			{return hash(str);}
		...
		public:
			...
			explicit FeatureDescriptor(const LV2_Feature* const* features)
				...
				for(size_t k=0;k<FeatureRequest::size();++k)
					{m_id[k]=feature_map->map(feature_map->handle,FeatureRequest::get(*this,k));
				}	
